  ...
  app.innerHTML=shell(`
    <div class="landing2">
      <section class="l-hero">
        ...
        <div class="l-guide-link"><a href="/guide.html" target="_blank" rel="noopener">New to Verve? Read the 2-minute teacher guide →</a></div>
        <div class="l-device">
          <div class="l-screen">
...
function syncEditors(){
  ...
  document.querySelectorAll("[data-i]").forEach(card=>{
    ...
    // Multipart part with an un-typeable fraction answer. Parts render as plain
    // typed boxes with no "/" key and no fraction/option UI, so an "n/d" part
    // answer cannot be entered by the pupil. We can't silently switch a part to
    // a type the part UI doesn't render, so flag the whole question for teacher
    // review and mark which parts need rewording (usually: ask for a decimal).
    if(q.type==="multipart" && Array.isArray(q.parts)){
      // A part whose answer is a slash-fraction "n/d" can't be typed on a number
      // pad, so switch it to the dedicated FRACTION input (two boxes with a fixed
      // "/"), which the pupil view now renders for parts. Only auto-switch a
      // part the teacher hasn't manually set to another type.
      q.parts.forEach(p=>{
        const t=p.type||"number";
        if((t==="number"||t==="")&&!p.type_user_set&&/^\s*-?\d+\s*\/\s*\d+\s*$/.test(String(p.answer||""))){
          p.type="fraction";
        }
      });
      // Any slash-fraction parts still left as number (e.g. teacher forced number)
      // are un-typeable — keep the warning for those only.
...
window.myPrizesPage=function(){
  ...
  let p=`<div class="mission"><h1>${esc(state.studentName)}'s Prize Gallery</h1><p class="muted">Your achievements and unlocked badges</p></div><div class="card gems-summary"><div class="gems-summary-content"><span class="gem-total">💎 ${g} gems</span><span class="badges-count">${a.badges.length} badges collected</span></div></div><div class="prizes-section"><h2>🏆 Achievement Badges</h2><div class="badges-grid">`;
